...
import { Center, Divider, Flex, Group, NumberInput, Stack, Text, TextInput, createStyles } from '@mantine/core';
...
const BuyForm = ({ form }: BuyFormProps) => {
  ...
  function handleSubmit(values) {
    ...
    setSubmit(true);
    dispatch(p2pActions.requestBuyCoin(payload));
  }
  ...
  useEffect(() => {
    if (totalCoin?.coin_id !== -1) setNumber(((totalCoin?.rate_usdt_coin * 0.01).toString().split('.')[1] || '').length);
  }, [totalCoin]);
  ...
  return (
    <>
      <form onSubmit={form.onSubmit(values => handleSubmit(values))}>
        <Stack>
          <TextInput
            label={t('P2P.Quantity want to buy')}
          ...
          <ContentTransaction form={form} type={1} />
          ...
          <Group className={classes.groupButtonStake}>
            <FilledButton h={50} className={classes.button} type="submit">
              ...
              >
                <p>{t('P2P.Confirm')}</p>
                <p>
                  ({t('P2P.Buy')} {Number(form.values.quantity).toLocaleString('vi-VN')} {totalCoin?.coin_name})
                </p>
              </Stack>
            </FilledButton>
          </Group>

          <ModalConfirm onContinue={handleContinueForm} opened={opened} setOpened={setOpened} type={1} form={form} />
        </Stack>
      </form>
    </>
  );
