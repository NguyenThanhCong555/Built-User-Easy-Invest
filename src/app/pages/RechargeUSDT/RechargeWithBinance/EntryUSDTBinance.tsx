...
import { selectLoadingRechargeUsdt } from 'store/slice/recharge/selector';
...
export const EntryUSDTBinance = (props: Props) => {
  ...
  const calledListBankBinance = useSelector(selectCalledListBankBinance);
  const listBankBinance = useSelector(selectListBankBinance);
  const loadingRechargeUsdt = useSelector(selectLoadingRechargeUsdt);
  ...
  useEffect(() => {
    if (!calledListBankBinance) {
      dispatch(rechargeActions.requestGetListBankBinance());
    }
  }, []);
  ...
  useEffect(() => {
    if (errorRecharge === RESPONSE_SUCCESS_ERROR) {
      ...
      navigation('/recharge/binance/complete', { state: { usdt } });
    }
    return () => {
  ...
  return (
    <>
      ...
      <Group position="apart" noWrap mt={8}>
        ...
        <Text className="body_4-bold">{usdt ? numberWithCommas(Number(usdt)) : 0} USDT</Text>
      </Group>

      <Group className={classes.groupButtonContinue}>
        <MyButton loading={loadingRechargeUsdt} onClick={handleContinue} w="100%" width_mobile="100%" h={44}>
