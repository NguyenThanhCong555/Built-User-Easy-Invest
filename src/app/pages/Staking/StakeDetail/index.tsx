...
import {
  selectBoughtStakes,
  selectCalledBoughtStakes,
  selectLoadingWithdrawStake,
  selectResponseErrorWithdrawStake,
...
import { PopupWithdrawConfirmation } from '../components/Popup';
...
export const StakedDetail = () => {
  ...
  const responseErrorWithdrawStake = useSelector(selectResponseErrorWithdrawStake);
  const loadingWithdrawStake = useSelector(selectLoadingWithdrawStake);
  ...
  useEffect(() => {
    if (responseErrorWithdrawStake === RESPONSE_SUCCESS_ERROR) {
      dispatch(stakeActions.resetResponseWithdrawStake());
  ...
  useEffect(() => {
    ...
    if (projectDetail?.state === CLOSE) {
      ...
      navigation(`/projects/stakes/${stakingInfo?.project_id}`);
    }
  ...
  const handleWithdrawStake = () => {
    ...
    dispatch(stakeActions.requestWithdrawStake({ stakingId: stakingInfo.id }));
  };
