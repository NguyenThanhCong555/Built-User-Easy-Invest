...
export const EntryUSDT = (props: Props) => {
  ...
  const handleConfirm = () => {
    if (isNaN(Number(usdt)) || Number(usdt) <= MIN_WITHDRAW) {
      setErrorEmpty(true);
    ...
    navigation('/withdraw/confirm', { state: { usdt } });
  };
  ...
  return (
    <Frame onMovePage={moveToChooseMethodWithdraw} titlePage={t('Withdraw.titlePage')}>
      <Box px={16}>
        ...
        >
          <TextInput
            placeholder={`${t('Withdraw.entry_value')} ${MIN_WITHDRAW}-${numberWithCommas(minimumUsdt.toFixed(3))}`}
            value={usdt}
        ...
        {errorEmpty && (
          <Text className="small_3-regular" c={errorEmpty ? variable.secondary.secondary2 : variable.neutral.grey}>
            {t('Withdraw.errorUSDT')} {MIN_WITHDRAW} - {numberWithCommas(minimumUsdt.toFixed(3))} USDT
          </Text>
        )}
